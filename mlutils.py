...
def search_google_build_dataset_stage2(k1, k2, train_pct, n_images, downloads, traintest_imgs):
    ...
    classdirs = [i for i in classdirs if i.strip()!=""]
    ...
    for classdir in classdirs:
        ...
        files = run_bash("find %s/%s -type f"%(downloads, classdir))
...
def show_image_sample(traintest_imgs, img_size=32):
    ...
    for imgs,labels in train_generator_fn():
        if np.random.random()>.5:
            break
    show_labeled_image_mosaic(imgs, labels)
